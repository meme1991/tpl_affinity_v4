...
class OverrideGeraintModLatestView extends DefaultModLatestView
{
	...
	function displayLatestEvents(){
		...
		$content = "<div class='mod_jevents_latest footer-block'>";

		if(isset($this->eventsByRelDay) && count($this->eventsByRelDay)){

			$content .= $this->modparams->get("modlatest_templatetop") || $this->modparams->get("modlatest_templatebottom") ? $this->modparams->get("modlatest_templatetop") : '<ul class="list-group ">';
			...
			foreach($this->eventsByRelDay as $relDay => $daysEvents){
				...
				foreach($daysEvents as $dayEvent){
					...
					if($firstTime) $eventrow = '<li class="list-group-item mod_events_latest_first mb-2 px-3" style="'.$dst.'">%s'."</li>\n";
					else $eventrow = '<li class="list-group-item mod_events_latest mb-2 px-3" style="'.$dst.'">%s'."</li>\n";
